...
class TestRetrieveChannelPlaylists(TestCase):
    ...
    def test_return_channel_playlists_with_visibility_PUBLIC(self):
        channel_playlists: list[Playlist] = PlaylistFactory.create_batch(
            2,
            channel=self.channel, visibility=Visibility.PUBLIC
        )
        not_own_playlist: Playlist = PlaylistFactory.create(visibility=Visibility.PUBLIC)

        response = self.client.get(self.url)

        retrieved_channel_playlists = [dict(playlist)['id'] for playlist in response.data.get('data')]
    ...
    def test_return_serialized_channel_playlists(self):
        channel_playlist = PlaylistFactory.create(channel=self.channel, visibility=Visibility.PUBLIC)
        ...
        self.assertDictEqual(response.data.get('data')[0], serialized_channel_playlist.data)
